...
func _init():
	print_debug("SW Init timestamp: " + str(SWUtils.get_timestamp()))


func _ready():
	# The following line would keep SilentWolf working even if the game tree is paused.
	#pause_mode = Node.PAUSE_MODE_PROCESS
	print_debug("SW ready start timestamp: " + str(SWUtils.get_timestamp()))
	...
	print_debug("SW ready end timestamp: " + str(SWUtils.get_timestamp()))
...
func send_get_request(http_node: HTTPRequest, request_url: String):
	...
	print_debug("GET headers: " + str(headers))
...
func send_post_request(http_node, request_url, payload):
	...
	headers = add_jwt_token_headers(headers)
	print_debug("POST headers: " + str(headers))
